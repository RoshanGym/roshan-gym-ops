...
const ITEM_RULES = [
  ...
  { match: /treadmill/i,                    category: 'OTHERS', note: 'Treadmill' },
  { match: /studio\s*rent/i,                category: 'OTHERS', note: 'Studio rent' },
  { match: /key\s*fob|keyfob|access\s*card/i, category: 'OTHERS', note: 'Access card' },
  { match: /membership|access\s*card/i,     category: 'OTHERS', note: 'Membership' },
];
...
// Items that ARE real sales even when the POS files them under Products/Inventory.
// Keyfob / access card is a service sale (tracked as OTHERS), not merchandise.
const KEEP_AS_SALE_ITEMS = [/key\s*fob|keyfob|access\s*card/i];
...
function isKeptSaleItem(item) {
  return KEEP_AS_SALE_ITEMS.some((r) => r.test(item || ''));
}
// Whether a POS line should be EXCLUDED from sales. Prefer this over
// isExcludedAvailment when you have the item name — it keeps keyfob/access-card
// lines (a real sale) even though the POS files them under Products/Inventory.
function isExcludedLine(availment, item) {
  if (isKeptSaleItem(item)) return false;
  return isExcludedAvailment(availment);
}
